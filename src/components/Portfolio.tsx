      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredItems.map((item, index) => {
            const firstImage = item.project_media?.find(m => m.type === 'image')?.url;
            const imageToShow =
              firstImage ||
              item.image_url ||
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <div className="absolute bottom-0 left-0 right-0 p-6">
                    ...
                    <div className="flex gap-3">
                      <button
                        className="flex items-center gap-2 bg-almanbar-gold text-almanbar-navy px-4 py-2 rounded-full font-semibold hover:bg-almanbar-gold-dark transition-colors duration-300"
                      ...
                      <button
                        className="flex items-center gap-2 bg-white/20 text-white px-4 py-2 rounded-full font-semibold hover:bg-white/30 transition-colors duration-300"
                        onClick={() => navigate(`/projects/${item.id}`)}
                      >
